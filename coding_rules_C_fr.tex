...
\chapter{C}
	\section{Projet}
		...
		\subsection{Pas d'IDE}
			...
			\subsubsection{Sous arborescence}
				...
				Le nombre de sous répertoires ainsi que leurs profondeur n'est pas limité, mais ne doit pas nuire à la compréhension du code.

	\section{Fichiers}
		\subsection{Source.c}
			Un ficher source est un fichier d'extension \verb+*.c+ ou \verb+*.cpp+; il doit forcement être accompagné d'un fichier header avec l'extension \verb+*.h+ (à l'exception du main qui peut s'en dispenser).
			...
			Un fichier sources peut avoir plusieurs headers.

		\subsection{Header.h}
			Tous les fichiers headers doivent avoir des barrières d'inclusions (protection contre les inclusions récursives).
			
			Les barrières d'inclusions portent le nom du fichier, sans espaces, en MAJUSCULE, les points sont remplacés par des << \verb+_+ >> et elles commencent et ce terminent par un double underscroe << \verb+__+ >>.
		
			Un fichier d'en-tête doit avoir le même nom que le fichier source afférant, à l'exception de l'extension en *.h. Si un fichier source à plusieurs fichier d'en-tête, ils doivent clairement lui faire référence de par leur dénomination.
	...
	\section{Define \& macros}
		...
		\subsection{Typage}
			...
			Pour les fonctions macroscopiques, elles peuvent être remplacées par des fonctions \verb+inline+. Cela permet la vérification des types et une plus grande sécurité quand à des effets de bord indésirables :
	...
	\section{Variables}
		\subsection{Typographie}
			...
			Les variables doivent avoir un nom de plus de deux lettres (excepté pour les compteurs de boucles locales, ils doivent néanmoins être commentés lors de leur déclaration).
		...
		\subsection{Déclaration}
			Les variables doivent être déclaré de façon à avoir la porté la plus courte possible, ainsi elles devront être déclaré juste avant leur utilisation.
		...
		\subsection{Initialisation}
			Une variable, de même qu'un tableau mémoire, doit toujours être initialisée \sout{en début de programme} lors de sa déclaration.
			...
			Même si par défaut beaucoup de compilateur initialise la mémoire à 0, il arrive que lors d'appels multiples à une fonction, une même variable se retrouve sur sa précédente zone mémoire et récupère sa précédente valeur. Il est donc nécessaire de fixer la valeur d'initialisation.
		...
		\subsection{Portée}
			Les variables doivent être déclaré de façon à avoir la portée la plus courte possible, ainsi cette structure : 
	...
	\section{Fonctions}
		\subsection{Typographie}
			...
			Si une ou plusieurs fonction appartiennent à un \verb+objet+/\verb+class+ alors on pourra préfixer le nom de la fonction par le nom de l'objet avec un undersocre pour séparer les deux \verb+strings+.
		...
		\subsection{portée}
			Les fonctions dont la portée est limitée au fichier lui-même doivent être définies en \verb+static+, et ne doivent pas être déclarés dans le fichier header correspondant.
	...
	\section{Général}
		...
		\subsection{structures}
			Les structures utilisées que dans un seul et unique fichier source doivent être définies dans ce même fichier source.
	...
	\section{Aide \& usage}
		Vous devez quand vous écrivez un programme. Toujours ajouter un affichage d'aide quant à l'utilisation du dit programme, cet affichage sera aussi affiché en cas d'appel incorrect au programme.

		L'aide doit être affiché dans tous les cas ou, le programme est appelé avec le mauvais nombre d'arguments, une commande invalide ou la commande d'aide.
		...
		\begin{itemize}
			...
			\item toutes autres informations relatives aux arguments
		\end{itemize}
		...
		Cette commande utilise le contenu de \verb+input_file+ comme entrée standard plutôt que le \verb+stdin+ et utilise le fichier \verb+output_file+ à la place de \verb+stdout+. Pour cette raison les opérateur \verb+< >+ sont réservés aux redirection.
	...
	\section{Documentation}
		...
		Pour générer le Doxygen dans une seule langue, vous devrez dans le fichier Doxyfile définir quelle langue doit être utiliser pour la génération :
